...
const courseId=params.courseId
  ...
  const availableTimes = selectedDay
    ? availabilityData.find((slot) => slot.day === selectedDay)?.times.filter((time) => time.bookedBy.studentId === null)
    : [];
  ...
  {courseData.endDate && !isNaN(new Date(courseData.endDate)) 
    ? new Date(courseData.endDate).toLocaleDateString([], { year: 'numeric', month: 'short', day: 'numeric' })
    : 'N/A'}
</p>
              ...
              <div className="w-1/2 flex flex-col gap-3 border border-gray-400 rounded-xl p-3 shadow-lg">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      >
                        ...
                        {availableTimes.map((time) => (
                          ...
                          >
                            <span>{new Date(time.start).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} - {new Date(time.end).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                            </option>
                        ))}
                      </select>
                    </div>
                ...
                <button
                  className={`mt-4 p-2 rounded-md ${selectedDay && selectedTime ? "bg-blue-500 text-white" : "bg-gray-300 text-gray-500 cursor-not-allowed"}`}
  ...
  {courseData.meetings && courseData.meetings.length > 0 ? (
    courseData.meetings
      .filter(meeting => new Date(meeting.date) < new Date()).length > 0 ? (
        courseData.meetings
          ...
          .map((meeting, index) => (
            <div key={index} className="flex justify-between">
              ...
              <p className="text-gray-600">
                {new Date(meeting.date).toLocaleDateString([], { year: 'numeric', month: 'short', day: 'numeric' })} | {meeting.time}
              </p>
            </div>
          ))
      ) : (
